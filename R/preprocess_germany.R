...
if (date_1 != date_2) {
  # read the shapefile
  germany_shape <- read_sf("germany_data/shapefile_germany.shp")
  cols <- unlist(read_csv("germany_data/colnames_shapefile.csv"))
  colnames(germany_shape) <- c(cols[cols != "geometry"], "geometry")
...
# ]
newest_numbers <- germany[
  germany$Date == as.Date("2021-05-02"),
]

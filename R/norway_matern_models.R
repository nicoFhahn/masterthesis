...
formula_1 <- value ~
  ...
  f(idarea_1, model = "matern2d", graph = g, ncol = 59, nrow = 356, hyper = prior_1) +
  # random effects term
...
formula_2 <- value ~
  ...
  pop_dens + urb_dens + sex + median_age +
  # specify the model with neighborhood matrix
  f(idarea_1, model = "matern2d", graph = g, ncol = 59, nrow = 356, hyper = prior_2) +
  # random effects term
  f(idarea_2, model = "iid")
...
saveWidget(
  map,
  paste(
    "html_plots/norway_matern_model_",

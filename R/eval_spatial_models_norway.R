...
options(scipen = 10)
models_nospatial[[3]]$summary.fixed[
  order(models_nospatial[[3]]$summary.fixed$mean),
]
sapply(
  models_nospatial[[3]]$marginals.fixed[
    rownames(models_nospatial[[3]]$summary.fixed[
      order(models_nospatial[[3]]$summary.fixed$mean),
    ])
  ],
  inla.emarginal,
...
options(scipen = 10)
models_nospatial[[15]]$summary.fixed[
  order(models_nospatial[[15]]$summary.fixed$mean),
]
sapply(
  models_nospatial[[15]]$marginals.fixed[
    rownames(models_nospatial[[15]]$summary.fixed[
      order(models_nospatial[[15]]$summary.fixed$mean),
    ])
  ],
  inla.emarginal,
...
newest_numbers$rr <- models_nospatial[[14]]$summary.fitted.values$mean
csi <- models_nospatial[[14]]$marginals.random$idarea_1[
...
m <- models_nospatial[[14]]$marginals.random$idarea_1
...
var_v <- inla.rmarginal(
  ...
  inla.tmarginal(
    function(x) 1 / x,
    models_nospatial[[14]]$marginals.hyperpar$`Precision for idarea_1`
  )
)
...
options(scipen = 10)
models_besag[[17]]$summary.fixed[
  order(models_besag[[17]]$summary.fixed$mean),
]
sapply(
  models_besag[[17]]$marginals.fixed[
    rownames(models_besag[[17]]$summary.fixed[
      order(models_besag[[17]]$summary.fixed$mean),
    ])
  ],
  inla.emarginal,
...
newest_numbers$rr <- models_besag[[17]]$summary.fitted.values$mean
csi <- models_besag[[17]]$marginals.random$idarea_1[
  seq_len(nrow(newest_numbers))
]
a <- 0
prob_csi <- lapply(csi, function(x) {
...
newest_numbers$prob_csi <- cat_csi
mat_marg <- matrix(NA, nrow = nrow(newest_numbers), ncol = 100000)
m <- models_besag[[17]]$marginals.random$idarea_1
for (i in seq_len(nrow(newest_numbers))) {
  u <- m[[i]]
  mat_marg[i, ] <- inla.rmarginal(100000, u)
}
...
var_v <- inla.rmarginal(
  ...
  inla.tmarginal(
    function(x) 1 / x,
    models_besag[[17]]$marginals.hyperpar$`Precision for idarea_1`
  )
)
perc_var_u <- mean(var_u / (var_u + var_v))
perc_var_u

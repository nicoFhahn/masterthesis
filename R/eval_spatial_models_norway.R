...
demo_results <- c(results_nospatial[1:6])
...
id_nospatial <- which(unlist(mae_bym2[1:11]) %in% min(unlist(mae_bym2[1:11])))
...
infra_dic[id_nospatial - 11]
infra_waic[id_nospatial - 11]
infra_cpo[id_nospatial - 11]
mae_nospatial[id_nospatial]
models_nospatial[[id_nospatial]]$summary.fixed[
  order(models_nospatial[[id_nospatial]]$summary.fixed$mean),
]
...
id_nospatial <- which(unlist(mae_bym2[14:15]) %in% min(unlist(mae_bym2[14:15]))) + 13
...
id_besag <- which(unlist(mae_besag[14:15]) %in% min(unlist(mae_besag[14:15])))
all_dic[id_besag]
all_waic[id_besag]
all_cpo[id_besag]
mae_besag[id_besag + 13]
id_bym2 <- which(unlist(mae_bym2[14:15]) %in% min(unlist(mae_bym2[14:15])))
all_dic[id_bym2 + 2]
all_waic[id_bym2 + 2]
all_cpo[id_bym2 + 2]
mae_bym2[id_bym2 + 13]
id_leroux <- which(
  unlist(mae_leroux[14:15]) %in% min(unlist(mae_leroux[14:15]))
)
all_dic[id_leroux + 4]
all_waic[id_leroux + 4]
all_cpo[id_leroux + 4]
mae_leroux[id_leroux + 13]
options(scipen = 10)
models_bym2[[id_bym2 + 13]]$summary.fixed[
  order(models_bym2[[id_bym2 + 13]]$summary.fixed$mean),
]
sapply(
  models_bym2[[id_bym2 + 13]]$marginals.fixed[
    rownames(models_bym2[[id_bym2 + 13]]$summary.fixed[
      order(models_bym2[[id_bym2 + 13]]$summary.fixed$mean),
    ])
  ],
  inla.emarginal,
...
newest_numbers$rr <- models_bym2[[id_bym2 + 13]]$summary.fitted.values$mean
csi <- models_bym2[[id_bym2 + 13]]$marginals.random$idarea_1[
  seq_len(nrow(newest_numbers))
]
a <- 0
prob_csi <- lapply(csi, function(x) {
...
newest_numbers$prob_csi <- cat_csi
mat_marg <- matrix(NA, nrow = nrow(newest_numbers), ncol = 100000)
m <- models_bym2[[id_bym2 + 13]]$marginals.random$idarea_1
for (i in seq_len(nrow(newest_numbers))) {
  u <- m[[i]]
  mat_marg[i, ] <- inla.rmarginal(100000, u)
}
...
var_v <- inla.rmarginal(
  ...
  inla.tmarginal(
    function(x) 1 / x,
    models_bym2[[id_bym2 + 13]]$marginals.hyperpar$`Precision for idarea_1`
  )
)
perc_var_u <- mean(var_u / (var_u + var_v))
perc_var_u

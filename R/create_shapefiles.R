...
norge_nursing_home <- norge_nursing_home[!duplicated(norge_nursing_home$osm_id, incomparables = NA), ]
norge_nursing_home <- norge_nursing_home[!rownames(norge_nursing_home) %in% rownames(norge_nursing_home[is.na(norge_nursing_home$osm_id), ][duplicated(st_coordinates(norge_nursing_home[is.na(norge_nursing_home$osm_id), ])), ]), ]
...
norge_restaurant <- norge_restaurant[!duplicated(norge_restaurant$osm_id, incomparables = NA), ]
norge_restaurant <- norge_restaurant[!rownames(norge_restaurant) %in% rownames(norge_restaurant[is.na(norge_restaurant$osm_id), ][duplicated(st_coordinates(norge_restaurant[is.na(norge_restaurant$osm_id), ])), ]), ]
...
norge_aerodrome <- norge_aerodrome[!duplicated(norge_aerodrome$osm_id, incomparables = NA), ]
norge_aerodrome <- norge_aerodrome[!rownames(norge_aerodrome) %in% rownames(norge_aerodrome[is.na(norge_aerodrome$osm_id), ][duplicated(st_coordinates(norge_aerodrome[is.na(norge_aerodrome$osm_id), ])), ]), ]
norge_aerodrome$type <- "aerodrome"
load("osmdata/norge_office.Rda")
norge_office <- st_as_sf(dplyr::bind_rows(norge_office))
norge_office <- norge_office[!duplicated(norge_office$osm_id, incomparables = NA), ]
norge_office <- norge_office[!rownames(norge_office) %in% rownames(norge_office[is.na(norge_office$osm_id), ][duplicated(st_coordinates(norge_office[is.na(norge_office$osm_id), ])), ]), ]
norge_office$type <- "office"
load("osmdata/norge_shops.Rda")
norge_shops <- st_as_sf(dplyr::bind_rows(norge_shops))
norge_shops <- norge_shops[!duplicated(norge_shops$osm_id, incomparables = NA), ]
norge_shops <- norge_shops[!rownames(norge_shops) %in% rownames(norge_shops[is.na(norge_shops$osm_id), ][duplicated(st_coordinates(norge_shops[is.na(norge_shops$osm_id), ])), ]), ]
norge_shops$type <- "shop"
load("osmdata/norge_platform.Rda")
norge_platform <- st_as_sf(dplyr::bind_rows(norge_platform))
norge_platform <- norge_platform[!duplicated(norge_platform$osm_id, incomparables = NA), ]
norge_platform <- norge_platform[!rownames(norge_platform) %in% rownames(norge_platform[is.na(norge_platform$osm_id), ][duplicated(st_coordinates(norge_platform[is.na(norge_platform$osm_id), ])), ]), ]
norge_platform$type <- "platform"
load("osmdata/norge_university.Rda")
norge_university <- st_as_sf(do.call(rbind, norge_university))
norge_university <- norge_university[!duplicated(norge_university$osm_id, incomparables = NA), ]
norge_university <- norge_university[!rownames(norge_university) %in% rownames(norge_university[is.na(norge_university$osm_id), ][duplicated(st_coordinates(norge_university[is.na(norge_university$osm_id), ])), ]), ]
norge_university$type <- "higher education"
load("osmdata/norge_college.Rda")
norge_college <- st_as_sf(do.call(rbind, norge_college))
norge_college <- norge_college[!duplicated(norge_college$osm_id, incomparables = NA), ]
norge_college <- norge_college[!rownames(norge_college) %in% rownames(norge_college[is.na(norge_college$osm_id), ][duplicated(st_coordinates(norge_college[is.na(norge_college$osm_id), ])), ]), ]
...
norge_kindergarten <- norge_kindergarten[!duplicated(norge_kindergarten$osm_id, incomparables = NA), ]
norge_kindergarten <- norge_kindergarten[!rownames(norge_kindergarten) %in% rownames(norge_kindergarten[is.na(norge_kindergarten$osm_id), ][duplicated(st_coordinates(norge_kindergarten[is.na(norge_kindergarten$osm_id), ])), ]), ]
norge_kindergarten$type <- "kindergarten"
load("osmdata/norge_schools.Rda")
norge_schools <- st_as_sf(dplyr::bind_rows(norge_schools))
norge_schools <- norge_schools[!duplicated(norge_schools$osm_id, incomparables = NA), ]
norge_schools <- norge_schools[!rownames(norge_schools) %in% rownames(norge_schools[is.na(norge_schools$osm_id), ][duplicated(st_coordinates(norge_schools[is.na(norge_schools$osm_id), ])), ]), ]
norge_schools$type <- "school"
load("osmdata/norge_bakery.Rda")
norge_bakeries <- st_as_sf(do.call(rbind, norge_bakeries))
norge_bakeries <- norge_bakeries[!duplicated(norge_bakeries$osm_id, incomparables = NA), ]
norge_bakeries <- norge_bakeries[!rownames(norge_bakeries) %in% rownames(norge_bakeries[is.na(norge_bakeries$osm_id), ][duplicated(st_coordinates(norge_bakeries[is.na(norge_bakeries$osm_id), ])), ]), ]
norge_bakeries$type <- "bakery"
load("osmdata/norge_hairdresser.Rda")
norge_hairdresser <- st_as_sf(dplyr::bind_rows(norge_hairdresser))
norge_hairdresser <- norge_hairdresser[!duplicated(norge_hairdresser$osm_id, incomparables = NA), ]
norge_hairdresser <- norge_hairdresser[!rownames(norge_hairdresser) %in% rownames(norge_hairdresser[is.na(norge_hairdresser$osm_id), ][duplicated(st_coordinates(norge_hairdresser[is.na(norge_hairdresser$osm_id), ])), ]), ]
norge_hairdresser$type <- "hairdresser"
load("osmdata/norge_clinic.Rda")
norge_clinic <- st_as_sf(dplyr::bind_rows(norge_clinic))
norge_clinic <- norge_clinic[!duplicated(norge_clinic$osm_id, incomparables = NA), ]
norge_clinic <- norge_clinic[!rownames(norge_clinic) %in% rownames(norge_clinic[is.na(norge_clinic$osm_id), ][duplicated(st_coordinates(norge_clinic[is.na(norge_clinic$osm_id), ])), ]), ]
norge_clinic$type <- "clinic"
load("osmdata/norge_sport.Rda")
norge_sport <- st_as_sf(dplyr::bind_rows(norge_sport))
norge_sport <- norge_sport[!duplicated(norge_sport$osm_id, incomparables = NA), ]
norge_sport <- norge_sport[!rownames(norge_sport) %in% rownames(norge_sport[is.na(norge_sport$osm_id), ][duplicated(st_coordinates(norge_sport[is.na(norge_sport$osm_id), ])), ]), ]
...
norge_entertainment <- norge_entertainment[!duplicated(norge_entertainment$osm_id, incomparables = NA), ]
norge_entertainment <- norge_entertainment[!rownames(norge_entertainment) %in% rownames(norge_entertainment[is.na(norge_entertainment$osm_id), ][duplicated(st_coordinates(norge_entertainment[is.na(norge_entertainment$osm_id), ])), ]), ]
...
norge_marketplace <- norge_marketplace[!duplicated(norge_marketplace$osm_id, incomparables = NA), ]
norge_marketplace <- norge_marketplace[!rownames(norge_marketplace) %in% rownames(norge_marketplace[is.na(norge_marketplace$osm_id), ][duplicated(st_coordinates(norge_marketplace[is.na(norge_marketplace$osm_id), ])), ]), ]
...
norge_residential <- norge_residential[!duplicated(norge_residential$osm_id, incomparables = NA), ]
norge_residential <- norge_residential[!rownames(norge_residential) %in% rownames(norge_residential[is.na(norge_residential$osm_id), ][duplicated(st_coordinates(norge_residential[is.na(norge_residential$osm_id), ])), ]), ]
...
germany_nursing_home <- germany_nursing_home[!duplicated(germany_nursing_home$osm_id, incomparables = NA), ]
germany_nursing_home <- germany_nursing_home[!rownames(germany_nursing_home) %in% rownames(germany_nursing_home[is.na(germany_nursing_home$osm_id), ][duplicated(st_coordinates(germany_nursing_home[is.na(germany_nursing_home$osm_id), ])), ]), ]
...
germany_restaurant <- germany_restaurant[!duplicated(germany_restaurant$osm_id, incomparables = NA), ]
germany_restaurant <- germany_restaurant[!rownames(germany_restaurant) %in% rownames(germany_restaurant[is.na(germany_restaurant$osm_id), ][duplicated(st_coordinates(germany_restaurant[is.na(germany_restaurant$osm_id), ])), ]), ]
...
germany_aerodrome <- germany_aerodrome[!duplicated(germany_aerodrome$osm_id, incomparables = NA), ]
germany_aerodrome <- germany_aerodrome[!rownames(germany_aerodrome) %in% rownames(germany_aerodrome[is.na(germany_aerodrome$osm_id), ][duplicated(st_coordinates(germany_aerodrome[is.na(germany_aerodrome$osm_id), ])), ]), ]
germany_aerodrome$type <- "aerodrome"
load("osmdata/germany_office.Rda")
germany_office <- st_as_sf(dplyr::bind_rows(germany_office))
germany_office <- germany_office[!duplicated(germany_office$osm_id, incomparables = NA), ]
germany_office <- germany_office[!rownames(germany_office) %in% rownames(germany_office[is.na(germany_office$osm_id), ][duplicated(st_coordinates(germany_office[is.na(germany_office$osm_id), ])), ]), ]
germany_office$type <- "office"
load("osmdata/germany_shops.Rda")
germany_shops <- st_as_sf(dplyr::bind_rows(germany_shops))
germany_shops <- germany_shops[!duplicated(germany_shops$osm_id, incomparables = NA), ]
germany_shops <- germany_shops[!rownames(germany_shops) %in% rownames(germany_shops[is.na(germany_shops$osm_id), ][duplicated(st_coordinates(germany_shops[is.na(germany_shops$osm_id), ])), ]), ]
germany_shops$type <- "shop"
load("osmdata/germany_platform.Rda")
germany_platform <- st_as_sf(dplyr::bind_rows(germany_platform))
germany_platform <- germany_platform[!duplicated(germany_platform$osm_id, incomparables = NA), ]
germany_platform <- germany_platform[!rownames(germany_platform) %in% rownames(germany_platform[is.na(germany_platform$osm_id), ][duplicated(st_coordinates(germany_platform[is.na(germany_platform$osm_id), ])), ]), ]
...
germany_university <- germany_university[!duplicated(germany_university$osm_id, incomparables = NA), ]
germany_university <- germany_university[!rownames(germany_university) %in% rownames(germany_university[is.na(germany_university$osm_id), ][duplicated(st_coordinates(germany_university[is.na(germany_university$osm_id), ])), ]), ]
...
germany_college <- germany_college[!duplicated(germany_college$osm_id, incomparables = NA), ]
germany_college <- germany_college[!rownames(germany_college) %in% rownames(germany_college[is.na(germany_college$osm_id), ][duplicated(st_coordinates(germany_college[is.na(germany_college$osm_id), ])), ]), ]
...
germany_kindergarten <- germany_kindergarten[!duplicated(germany_kindergarten$osm_id, incomparables = NA), ]
germany_kindergarten <- germany_kindergarten[!rownames(germany_kindergarten) %in% rownames(germany_kindergarten[is.na(germany_kindergarten$osm_id), ][duplicated(st_coordinates(germany_kindergarten[is.na(germany_kindergarten$osm_id), ])), ]), ]
...
germany_schools <- germany_schools[!duplicated(germany_schools$osm_id, incomparables = NA), ]
germany_schools <- germany_schools[!rownames(germany_schools) %in% rownames(germany_schools[is.na(germany_schools$osm_id), ][duplicated(st_coordinates(germany_schools[is.na(germany_schools$osm_id), ])), ]), ]
germany_schools$type <- "school"
load("osmdata/germany_bakery.Rda")
germany_bakeries <- st_as_sf(do.call(rbind, germany_bakeries))
germany_bakeries <- germany_bakeries[!duplicated(germany_bakeries$osm_id, incomparables = NA), ]
germany_bakeries <- germany_bakeries[!rownames(germany_bakeries) %in% rownames(germany_bakeries[is.na(germany_bakeries$osm_id), ][duplicated(st_coordinates(germany_bakeries[is.na(germany_bakeries$osm_id), ])), ]), ]
...
germany_hairdresser <- germany_hairdresser[!duplicated(germany_hairdresser$osm_id, incomparables = NA), ]
germany_hairdresser <- germany_hairdresser[!rownames(germany_hairdresser) %in% rownames(germany_hairdresser[is.na(germany_hairdresser$osm_id), ][duplicated(st_coordinates(germany_hairdresser[is.na(germany_hairdresser$osm_id), ])), ]), ]
...
germany_clinic <- germany_clinic[!duplicated(germany_clinic$osm_id, incomparables = NA), ]
germany_clinic <- germany_clinic[!rownames(germany_clinic) %in% rownames(germany_clinic[is.na(germany_clinic$osm_id), ][duplicated(st_coordinates(germany_clinic[is.na(germany_clinic$osm_id), ])), ]), ]
germany_clinic$type <- "clinic"
load("osmdata/germany_sport.Rda")
germany_sport <- st_as_sf(dplyr::bind_rows(germany_sport))
germany_sport <- germany_sport[!duplicated(germany_sport$osm_id, incomparables = NA), ]
germany_sport <- germany_sport[!rownames(germany_sport) %in% rownames(germany_sport[is.na(germany_sport$osm_id), ][duplicated(st_coordinates(germany_sport[is.na(germany_sport$osm_id), ])), ]), ]
...
germany_entertainment <- germany_entertainment[!duplicated(germany_entertainment$osm_id, incomparables = NA), ]
germany_entertainment <- germany_entertainment[!rownames(germany_entertainment) %in% rownames(germany_entertainment[is.na(germany_entertainment$osm_id), ][duplicated(st_coordinates(germany_entertainment[is.na(germany_entertainment$osm_id), ])), ]), ]
...
germany_marketplace <- germany_marketplace[!duplicated(germany_marketplace$osm_id, incomparables = NA), ]
germany_marketplace <- germany_marketplace[!rownames(germany_marketplace) %in% rownames(germany_marketplace[is.na(germany_marketplace$osm_id), ][duplicated(st_coordinates(germany_marketplace[is.na(germany_marketplace$osm_id), ])), ]), ]
...
germany_residential <- germany_residential[!duplicated(germany_residential$osm_id, incomparables = NA), ]
germany_residential <- germany_residential[!rownames(germany_residential) %in% rownames(germany_residential[is.na(germany_residential$osm_id), ][duplicated(st_coordinates(germany_residential[is.na(germany_residential$osm_id), ])), ]), ]
...
germany_shape$residential <- unlist(
  lapply(
    ...
    function(x, ...) {
      ...
      in_box <- germany_residential[(coordinates[, 1] > bbox[1] & coordinates[, 1] < bbox[3]) & (coordinates[, 2] > bbox[2] & coordinates[, 2] < bbox[4]), ]
      length(
        unlist(
          st_intersects(
            germany_shape[x, ],
